...
class ContentsResource extends AbstractResource
{
    ...
    protected function outputContentsMask($contents, $params, $query)
    {
        ...
        foreach ($contents as &$content) {
            ...
            if (isset($params["includetl"])){
                $taxoTermsService=Manager::getService("TaxonomyTerms");
                $content["taxoLabels"]=[];
                if (isset($content["taxonomy"]) && is_array($content["taxonomy"])){
                    foreach($content["taxonomy"] as $terms){
                        if(is_array($terms)) {
                            foreach($terms as $termId) {
                                $foundTerm=$taxoTermsService->findById($termId);
                                if ($foundTerm){
                                    $content["taxoLabels"][$termId]=$foundTerm["text"];
                                }
                            }
                        }
                    }
    ...
    {
        if($orderByTitle) $filters["sort"] = array(["property":"title","direction":"DESC"]);
