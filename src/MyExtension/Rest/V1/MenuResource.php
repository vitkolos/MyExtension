    ...
    protected function _getPagesByLevel($rootPage, $targetLevel, $currentLevel = 1, $locale,$params)
    {
        ...
        if ($currentLevel == $targetLevel) {
            foreach ($pages as $key => $page) {
                ...
                $pages[$key]= array_intersect_key($pages[$key], array_flip(array('title','description' ,'id', 'text','pages','url','eCTitle','eCDescription','eCImage','richTextId','includedRichText','taxonomy','orderValue','blocks')));//add blocks
            }
            return $pages;
        }
        foreach ($pages as $key => $page) {
