...
Ext.define('Rubedo.controller.DAMController', {
        ...
        form.submit({
            ...
            success: function(form, action) {
                ...
                if (Ext.getCmp("DAMCreateUpdateWindow").directContribute){
                    ...
                    var obtainedMCResponse=Ext.JSON.decode(action.response.responseText);
        Ext.getCmp(Ext.getCmp("searchResultsWindow").targetId).setValue(obtainedMCResponse.data.id);
    ...
    onPixlrEditorWindowAfterRender: function(component, eOpts) {
        ...
        var addHtml='<iframe id="pixlr" type="text/html" width="100%" height="1200px" src="https://pixlr.com/editor/?referrer=Rubedo&title='+targetedImageTitle+'&locktitle=true&method=GET&image='+imageUrl+'&target='+imageResponseUrl+'&locktarget=true&icon='+iconUrl+'&exit='+exitUrl+'" frameborder="0"></iframe>';
    ...
    fireUnitaryEdit: function(ROMode) {
        ...
        valueBox=Ext.Object.merge(valueBox,record.get("taxonomy"));
        myEditor.setTitle(Rubedo.RubedoAutomatedElementsLoc.DAMEditText+" \" "+record.get("title")+" \"");
        valueBox.writeWorkspace=record.get("writeWorkspace");
        valueBox.target=record.get("target");
        myEditor.getComponent(0).getForm().setValues(valueBox);
        Ext.getCmp("DAMCreateUpdateWindow").doLayout();
        if (ROMode){
