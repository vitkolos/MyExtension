angular.module("rubedoBlocks").lazy.controller('AddThisShareController',['$scope','$http','$location',function($scope,$http,$location){
    ...
    me.shareCounter=0;
        ...
        addthis.sharecounters.getShareCounts(['facebook', 'twitter'], function(obj) {
            for (var i = 0; i < 2; i++) {
                if(obj[i].service=="twitter") {me.shareCounter+=Number(obj[i].count); console.log(obj[i].count)}
                else if(obj[i].service=="facebook") {me.shareCounter+=Number(obj[i].total_count);console.log(obj[i].total_count)}
