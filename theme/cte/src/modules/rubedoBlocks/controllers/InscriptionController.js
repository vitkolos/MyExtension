    ...
    me.getFormulaire = function (contentId){
        RubedoContentsService.getContentById(contentId, options).then(function(response){
            if (response.data.success){
                ...
                if ( (me.form.questions.transport.length > 0) || ( (me.form.fields.transport)&&((me.form.fields.transport.transport).length>0))) {me.isTransport = true;}
                if ( ( (me.form.fields.logement)&&((me.form.fields.logement).length>0 )) || ((me.form.questions.logement.logement).length > 0) ) {me.isLogement = true;}
