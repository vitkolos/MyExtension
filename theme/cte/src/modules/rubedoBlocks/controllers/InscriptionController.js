    ...
    me.setCurrentStage = function(step, valide) {
        if (valide && (me.currentStage >= step)) {
            if (step==0) {me.currentStage=1;}
                ...
                else if (me.isComplement) {me.currentStage=2;$('#infos_complementaires').collapse({toggle:true,parent:'#inscription_form'});$scope.mailError = false;$scope.mailError2 = false;}
                else if (me.isTransport) {me.currentStage=3;$('#transport').collapse({toggle:true,parent:'#inscription_form'});$scope.mailError = false;$scope.mailError2 = false;}
